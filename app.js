...
const BACKEND_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
  ? 'http://localhost:3000'
  : (window.location.protocol === 'file:' ? 'http://localhost:3000' : 'http://13.51.167.62:3000');
...
function populateDeliveryDays() {
  const select = document.getElementById('order-day');
  if (!select) return;
  select.innerHTML = '';
  let startDate = new Date();
  const schoolStart = new Date('2026-09-14T00:00:00');
  if (startDate < schoolStart) {
    startDate = new Date(schoolStart);
  }
  let count = 0;
  ...
  while (count < 5) {
    ...
    const cutoffTime = new Date(tempDate);
    ...
    const dayString = tempDate.toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric', weekday: 'long' });
...
async function submitOrder() {
  ...
  const note = '';
  const deliveryDaySelect = document.getElementById('order-day');
  const deliveryDay = deliveryDaySelect.value;
  const deliveryDayText = deliveryDaySelect.options[deliveryDaySelect.selectedIndex] ? deliveryDaySelect.options[deliveryDaySelect.selectedIndex].text : deliveryDay;
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      document.getElementById('success-message-text').innerHTML = `Siparişiniz Alındı! Sipariş Numaranız: <strong>#${data.orderId}</strong>.<br><br>📍 Lütfen <strong>${escapeHTML(deliveryDayText)}</strong> günü, <strong>${escapeHTML(deliveryBreak)}</strong> vaktinde <strong>teslimat noktasına</strong> gelerek siparişinizi elden teslim alın.<br><br>💵 Ödemeyi teslimat anında elden yapmayı unutmayın.`;
